...
public class SettingsXML {
    ...
    public static void createGameFolder() {
        ...
        if (!gameFolder.exists()) {
            try {
                if (gameFolder.mkdir()) {
                    GameLogger.addLog("I", "A játék könyvtára létre lett hozva.", null);
                } else {
                    throw new SecurityException("gameFolder.mkdir() visszatérési értéke hamis.");
                }
            } catch (SecurityException se) {
                GameLogger.addLog("S", "Nem sikerült létrehozni a játék könyvtárát!", se);
            }
        } else {
            GameLogger.addLog("I", "A játék könyvtára már létezik!", null);
        }
    }
    ...
    public static void writeSettingsFileXML() {
        try {
            ...
            GameLogger.addLog("S", "A beállítások kiírása nem sikerült!", io);
        }
    }
}